...
from tonalytica_client import TonalyticaAPIClient, get_frontend_vals


def refresh_dashboard(baseurl, apikey, slug):

    client = TonalyticaAPIClient(apikey, baseurl)
    ...
    for idx, qry in queries_dict.items():
        ...
        r = client._post(f"queries/{idx}/results", json=body)
...
def get_queries_on_dashboard(client, slug):
    ...
    return {id: client.get(f"queries/{id}").json() for id in l_query_ids}


def fill_dynamic_val(dates, p):
    """Accepts parameter default information from the TONalytica API.
